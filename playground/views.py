...
from .utils import pwd_strength,predict_brain_cancer,predict_malaria,predict_pneumonia,save_img2s3
...
def new_report(request):
    ...
    #image_url=save_image_to_s3(img,image_name)
    image_array = np.frombuffer(file_content, np.uint8)
    decoded_image = cv2.imdecode(image_array, cv2.IMREAD_COLOR)
    img = cv2.cvtColor(decoded_image, cv2.COLOR_BGR2RGB)
    img=img_to_array(img)
    img=array_to_img(img)
    image_url=save_img2s3(img,image_name)
    
    

    # Create a file-like object from the bytes object
    #convert img to cv2 sendable
    img=file_content

    if disease_name=="Malaria":
        report_image,report_label,report_probability=predict_malaria(img,100)#sending url but non fetchable
    elif disease_name=="Brain Cancer":
        report_image,report_label,report_probability=predict_brain_cancer(img,240)
    elif disease_name=="Pneumonia":
        report_image,report_label,report_probability=predict_pneumonia(img,240)

    report_name=uuid.uuid4()
    report_url=save_img2s3(report_image,report_name)
    #report_probability=report_probability.tostring()
    #report_probability=report_probability.decode('latin-1')

    report_text=str(report_label)+" with a probability:"+str(report_probability)
    #get report image save it in asses put url in db 
    #getreport text also 
    model_obj=Report(Report_text=report_text,Report_URL=report_url,disease_name=disease_name,patient_id_id=id,doctor_id_id=doc_id,Image_URL=image_url)
    model_obj.save()
